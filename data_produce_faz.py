"""
Utility script to convert Retina FAZ project data into NPZ files compatible
...
For each case, this script creates one ``image_XXX.npz`` file that contains
``image.png`` and ``aux_1.png`` in the ``imgs`` and ``gts`` keys. Files
originating from the ``train`` split include ``train: True`` and ``val: False``
flags, while ``val`` split files set ``val: True`` and ``train: False``.

Usage example:
    python data_produce_faz.py \
        --root /home/wensheng/gjq_workspace/eyesam/data/Retina_Project_faz \
        --train-output /home/wensheng/gjq_workspace/eyesam/data/Retina_Project_faz/train_npz \
        --val-output /home/wensheng/gjq_workspace/eyesam/data/Retina_Project_faz/val_npz \
        --train-start 1 --val-start 1
"""
...
def process_split(
    ...
    for case_dir in case_dirs:
        ...
        try:
            mask_array = load_mask_grayscale(mask_path)
        except Exception as exc:  # pragma: no cover - defensive logging
            logging.error("Failed to load mask %s: %s", mask_path, exc)
            continue

        image_array, mask_array = validate_pair(image_array, mask_array, case_dir.name, mask_path.name)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert Retina FAZ PNGs to NPZ format for NPZRawDataset.")
    parser.add_argument(
        "--root",
        type=Path,
